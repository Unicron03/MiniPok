
local Branch = {img = love.graphics.newImage("assets/items/ressources/branch.png"), name = "Branch"}
Branch.__index = Branch

Branch.width = Branch.img:getWidth()
Branch.height = Branch.img:getHeight()

local FLOATING_AMPLITUDE = 5
local FLOATING_SPEED = 2.5

local ActiveBranchs = {}

function Branch.removeAll()
    for i,v in ipairs(ActiveBranchs) do
        v.physics.body:destroy()
    end

    ActiveBranchs = {}
end

function Branch:remove()
    for i, instance in ipairs(ActiveBranchs) do
        if instance == self then
            self.physics.body:destroy()
            table.remove(ActiveBranchs, i)
        end
    end
end

function Branch.new(x, y)
    instance = setmetatable({}, Branch)
    instance.x = x 
    instance.y = y
    instance.r = 0
    instance.scale = 2
    instance.itemType = ItemTypes.RESSOURCE

    instance.physics = {}
    instance.physics.body = love.physics.newBody(World, instance.x, instance.y, "static")
    instance.physics.shape = love.physics.newRectangleShape(instance.width * instance.scale, instance.height * instance.scale)
    instance.physics.fixture = love.physics.newFixture(instance.physics.body, instance.physics.shape)
    instance.physics.fixture:setSensor(true)
    table.insert(ActiveBranchs, instance)
end

function Branch:returnNbEntities()
    return #ActiveBranchs
end

function Branch:update(dt)
    -- self:syncPhysics()
end

function Branch:syncPhysics()
    self.x, self.y = self.physics.body:getPosition()
end

function Branch:draw()
    local offsetY = FLOATING_AMPLITUDE * math.sin(love.timer.getTime() * FLOATING_SPEED)
    local drawY = self.y + offsetY

    love.graphics.draw(self.img, self.x, drawY, self.r, self.scale, self.scale, self.width / 2, self.height / 2)

    -- self:drawHitbox()
end

function Branch:drawHitbox()
    love.graphics.setColor(1, 0, 0)
    love.graphics.polygon("line", self.physics.fixture:getBody():getWorldPoints(self.physics.fixture:getShape():getPoints()))
    love.graphics.setColor(1, 1, 1)
end

function Branch.updateAll(dt)
    for i,instance in ipairs(ActiveBranchs) do
        instance:update(dt)
    end
end

function Branch.drawAll()
    for i,instance in ipairs(ActiveBranchs) do
        instance:draw()
    end
end

function Branch.beginContact(a, b, collision)
    for i,instance in ipairs(ActiveBranchs) do
        if a == instance.physics.fixture or b == instance.physics.fixture then
            if a == Player.physics.fixture or b == Player.physics.fixture then
                Inventory:giveToInventory(instance.name, instance)
                instance:remove()
            end
        end
    end
end

return Branch